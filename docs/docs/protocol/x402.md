...
sequenceDiagram
  ...
  Note over A: Returns price + wallet address

  Note over C: Builds EIP-3009 signature

  C->>A: POST /execute + PAYMENT-SIGNATURE header
  A->>F: POST /verify
  F->>A: isValid + payer + payee

  Note over A: Runs your handler

  A->>C: 200 OK + output
  A-->>F: POST /settle (async)
  F-->>L: USDC.transferWithAuthorization()
  Note over L: USDC moves to agent wallet
```
