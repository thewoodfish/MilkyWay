# MILKYWAY_SHARING.md
## Agent Sharing Component — Full Spec
### For Claude Code

Read alongside all other MILKYWAY_*.md files.
File: `frontend/components/AgentShare.tsx`
...
```
Twitter / X    → developers. Primary audience.
Telegram       → crypto native users. Essential.
WhatsApp       → wide reach, emerging markets.
LinkedIn       → builders sharing their work professionally.
Copy Link      → always. Most used of all.
```

---

## Share Content Per Platform

Content is generated dynamically from agent data.
Each platform gets copy written for how people communicate there.
Never the same text copy-pasted across all platforms.

```typescript
interface AgentShareData {
  agentId: number;
  name: string;
  description: string;        // first sentence only
  priceEth: string;
  pricingModel: string;
  jobCount: number;
...
function buildShareUrls(content: ReturnType<typeof buildShareContent>) {
  return {
    ...
    // Note: LinkedIn doesn't support pre-filled text via share URL.
    // The URL is shared and LinkedIn pulls the OG preview.
    // The linkedin content string above is used for copy-to-clipboard fallback only.
  };
}
```
...
// Simple SVG icons — no icon library dependency
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <button
        ...
        className="
          h-8 rounded-lg border border-gray-200
...
## OG Image — Dynamic Per Agent

**File: `frontend/app/agents/[agentId]/opengraph-image.tsx`**

Generates a unique OG image per agent using `@vercel/og`.
This is what unfurls when the link is pasted into Twitter, Telegram, iMessage.
...
```bash
# frontend/.env.local — already exists, no new vars needed
# OG image uses NEXT_PUBLIC_API_URL to fetch agent data
```
...
- **Never open share URLs in the same tab.**
  Always `target="_blank" rel="noopener noreferrer"`.
- **Always encode share text with encodeURIComponent.**
  Special characters break share URLs silently.
- **LinkedIn does not support pre-filled text.**
  Only the URL is passed. The OG preview does the work.
  Never show an error because LinkedIn copy is "missing."
- **The copy fallback (execCommand) is for older browsers.**
  Keep it. navigator.clipboard fails silently in some contexts.
- **OG image is edge runtime — no Node.js APIs.**
  No fs, no path, no crypto. Only fetch and basic JS.
- **Never show the share row if agent is deactivated or down.**
  Sharing a dead agent wastes trust.
- **The "Copied!" state lasts exactly 2 seconds.**
  Not 1, not 3. 2. Enough to confirm, not annoying.

---

*MilkyWay Agent Sharing*
*Every share is distribution. Write copy worth sharing.*